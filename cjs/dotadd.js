...
class ADD {
  ...
  addOutputAndFillMatrix(out, gain) {
    ...
    this.decoder.output.matrix.forEach(ch => {
      while (ch.length != this.decoder.output.channels.length) ch.push(0);
    });
  }
  ...
  refitOutputChannels() {
    if (this.numOutputs() < this.totalMatrixOutputs()) {
      while (this.numOutputs() != this.totalMatrixOutputs()) this.addOutput(new OutputChannel('DEFAULT', 'default'));
    } else if (this.numOutputs() > this.totalMatrixOutputs()) {
      while (this.numOutputs() != this.totalMatrixOutputs()) this.decoder.output.channels.pop();
    }
  }
  ...
  refitOutputMatrix() {
    ...
    if (ol > this.numOutputs()) {
      ...
      this.decoder.output.matrix.map(ch => {
        ch.length = this.numOutputs();
        return ch;
      });
    } else if (ol < this.numOutputs()) {
      while (this.decoder.output.matrix.length != this.numOutputs()) this.decoder.output.matrix.push([]);

      this.decoder.output.matrix.map((ch, i) => {
        let l = ch.length;

        while (ch.length != this.numOutputs()) ch.push(0);
  ...
  validateDecodingMatrices() {
    ...
    for (let i in this.decoder.filter) {
      if (this.decoder.matrices.find(m => m.input == Number.parseInt(i)) == undefined) return this.invalidateWith('Missing matrix for filter output ' + i);
    }
